...
3. **R3 — Run Node, Main, and Evaluator through the local Codex CLI** → Story 3 / AC1–AC6.
   ...
   - A reviewed maintenance mode in the hardened worker setup owns image rebuild, exact optional auth-volume rotation, atomic owner/SYSTEM-only configuration replacement, and credential retirement. Before maintenance, an owner/SYSTEM-only process-lifetime control pipe lets the trusted operator client ask the running launcher to perform its existing authenticated T15 stop, container-absence proof, and bounded Funnel cleanup; forced task termination is never stop authority. Both modes preserve T15 KnownFolder, reparse, ACL, exact executable, named-pipe, label, container-absence, and process-environment boundaries; operators never move the secret configuration or remove the auth volume through ambient manual paths.
...
5. **R5 — Use free-tier-compatible background dispatch** → Story 5 / AC1–AC5 and Story 6 / AC1–AC4.
   ...
   - `GUSTAVO_HYBRID_BRIDGE_ENABLED` is the hosted direct-chat publication gate. There is no local runtime consumer for `GUSTAVO_MARKET_POLLER_ENABLED`, so that inert variable is removed rather than documented as a safety control. Market shutdown authority is pausing the exact QStash market schedule, then invoking the trusted operator client over the launcher's fixed owner/SYSTEM-only maintenance-control pipe. The running launcher alone uses its process-only HMAC nonce to close admission, settle already-admitted wake verification/enqueue work, run the existing bounded runtime/container stop proof, remove Funnel, and exit normally. Missing/late proof blocks cleanup; `Stop-ScheduledTask`, raw Funnel commands, and an unused environment variable are not stop authority. Durable `PENDING` jobs may remain for reconnect recovery; only claimed/active work must settle before local shutdown.
...
7. **R7 — Make cutover and recovery operable** → all stories.
   - Provide migration/bootstrap, Vercel env, Windows startup/operator-stop/maintenance, Funnel, quota, degraded-mode, rollback, backup, domain, and smoke instructions.
...
9. **Operational controls must be real authorities.** A clean reviewed commit plus trusted absolute Git/Node/Corepack/PowerShell executables and repository-pinned Vercel CLI own deployment commands. Hosted bridge disable is materialized in a staged/smoked/exact-promoted artifact. Recurring market disable is the exact schedule pause followed by the trusted `start-hybrid-worker.ps1 -StopForMaintenance` client. The normal launcher binds a fixed owner/SYSTEM-only process-lifetime control pipe before worker startup; a valid local request makes that already-running process use its private HMAC control nonce, stop new admissions, settle the admitted snapshot, prove runtime/container absence under the existing deadline, perform bounded Funnel teardown, exit normally, and return a safe proof. The client never receives the nonce and any timeout/missing proof blocks task cleanup, maintenance, and rollback. `Stop-ScheduledTask`, raw Funnel mutation, and an unused environment variable are not stop authorities. Local image/auth/config maintenance then runs only through the reviewed setup maintenance mode. Migration precedes the first authority-empty backup, and bootstrap follows that verified backup.
...
- `scripts/{setup-hybrid-worker,start-hybrid-worker}.ps1`, `infra/env.example`, and hybrid production tests: provision/validate the local-only materializer URL without printing or forwarding it to Vercel or the Codex container; setup adds a reviewed maintenance mode for safe image/auth/config rotation, while the launcher adds an owner/SYSTEM-only operator-stop pipe that invokes and proves the existing T15 stop before maintenance or rollback.
...
2. Pause/delete only the two mission-created QStash schedules. Durable `PENDING` jobs remain recoverable. Invoke the trusted `start-hybrid-worker.ps1 -StopForMaintenance` client and proceed only when the running launcher proves its admission snapshot settled, runtime/container absence, bounded Funnel teardown, normal exit, and exact root task non-running state. Never substitute `Stop-ScheduledTask` or a raw Funnel command.
...
- 2026-08-14 prompt-update: add a proven local operator-stop channel after regenerated T21 review showed `Stop-ScheduledTask` cannot invoke or validate the private T15 shutdown proof.
  - Previous intent: “exact schedule pause plus active-work settlement and T15 stop/absence proof owns market shutdown,” with the runbook calling `Stop-ScheduledTask` and raw Funnel commands.
  - Revised intent: the normal launcher owns a fixed owner/SYSTEM-only maintenance-control pipe; a trusted stop client asks that same process to use its private HMAC nonce, close admission, settle admitted work, prove runtime/container absence, tear Funnel down boundedly, and exit normally before returning a safe proof. Forced task termination and raw Funnel mutation are forbidden authorities.
- 2026-08-14 proven-local-worker-stop prompt update approved under the user's standing instruction to “Proceed with all without needed input”; new T20D plus regenerated T21/T22/T23 are required before execution resumes.
